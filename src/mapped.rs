...
use self::node_map::{LogicalAddress, Mapper, NodeMap};
...
enum UpperValues<K, V> {
    High {
        height: Height,
        values: Pointers<LogicalAddress>,
    },
    Low {
        ...
        K: Ord,
    {
        ...
        loop {
            match node.get(k) {
                ...
                Upper(addr) => node = self.mapper.get(&addr),
            }
        }
    }
        ...
        V: Clone,
    {
        if self.is_empty() {
            ...
            if child.slots_used() >= b as usize {
                let new_children = child.flush(b, empty());
